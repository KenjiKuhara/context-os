...
| ID | タスク | 内容 | ステータス |
|----|--------|------|------------|
| C1 | 3 モード切替 | ライト / ダーク / システム が意図どおり切替できる | DONE |
| C2 | 選択の保持 | 再読み込み後も選択が保持される | DONE |
| C3 | コントラスト（可読性） | 117 §2.2 が崩れていない（WCAG 2.1 AA を意識） | DONE |
| C4 | 重要状態色 | 成功 / 警告 / 危険 / 注目 がライト・ダークで見分けられる（117 §2.3） | DONE |
| C5 | 回帰なし | /dashboard の主要導線が壊れていない | DONE |
...
- **完了宣言**: Block A（A1〜A4）・Block B（B0〜B4）・Block C（C1〜C5）を完了し、MVP 合格とした。Phase12-Dark は **CLOSED（DONE）** とする。
- **参照ドキュメント**: 
  - **119** — 最終確認結果（Block C 証拠）
  - **118** — 本ドキュメント（実装タスク・契約・ゲート報告）
  - **117** — ダークモード設計
- **状態管理**: docs/114_phase_status.md に Phase12-Dark = CLOSED、Evidence = 117, 118, 119 を記録済み。次フェーズ候補（全ページ展開・high-contrast・アクセシビリティ）は 114 §8 に記載。

以上。Phase12-Dark の実装は 117 と本ドキュメント（118）に基づき、段階ゲートで進め、Block C の結果を 119 に証拠として残し正式にクローズした。
